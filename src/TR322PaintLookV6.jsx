/* ============================================================================
 * AVer TR322 — Camera & Paint/Look WEB UI 原型  (V6)
 * ----------------------------------------------------------------------------
 * 用途:Pro AV PTZ 攝影機 TR322 之 WEB UI「Paint/Look」色彩調校介面原型。
...
function TR322VideoAudioPage({ settings, onChange }) {
  ...
  const integratedOutputLicenseField = (
    <div className="aver-form-field tr322-output-license-field" style={{ ...fieldStyle, gridColumn: "span 2", border: `1.5px solid ${T.line}`, borderRadius: 4, overflow: "hidden", background: "#08090a", boxSizing: "border-box", width: "100%" }}>
      <div className="aver-form-field-header tr322-output-license-header" style={{ background: "#22252a", color: T.dim, fontFamily: fUI, fontWeight: 600, borderBottom: `1.5px solid ${T.line}` }}>
  ...
  return (
    <div id="aver-video-audio-wrapper" className="tr322-scroll-shell" style={{ position: "relative", width: "100%", height: "100%", overflowY: "auto", padding: "2px 4px 8px 0", boxSizing: "border-box", scrollbarWidth: "none" }}>
      <div className="tr322-layout-toolbar" style={{ position: "absolute", top: 7, right: 14, zIndex: 30, minHeight: 30, padding: "3px 4px 3px 8px", display: "flex", alignItems: "center", gap: 8, border: `1px solid ${T.line}`, borderRadius: 5, background: "rgba(24,27,31,.96)", boxShadow: "0 4px 12px rgba(0,0,0,.35)" }}>
...
export default function App() {
  ...
  // TR322 本次原型只開放 Video & Audio，其餘選單保留作為產品資訊架構提示。
    ...
    <div id="aver-paint-look-root" style={{ position: "relative", background: T.page, width: "100%", height: "100vh", fontFamily: fUI, color: T.text, display: "flex", overflow: "hidden" }}>
        ...
        {[
          ...
          return (
            <div 
              ...
              aria-disabled={!implement}
              title={implement ? lb : `${lb} is not included in this prototype`}
              onClick={() => { if (implement) setActiveMenu(id); }}
              style={{ 
                padding: "14px 24px", fontSize: 14, cursor: implement ? "pointer" : "not-allowed", 
                background: active ? T.sideActive : "transparent", 
                color: active ? "#fff" : implement ? T.dim : T.faint, 
                fontWeight: active ? 600 : 400, 
                borderLeft: active ? "4px solid #fff" : "4px solid transparent", 
                opacity: implement ? 1 : 0.48,
      ...
      <div id="aver-main-stage" style={{ position: "relative", flex: 1, padding: "16px 24px", minWidth: 0, background: T.page, overflow: "hidden", height: "100vh", display: "flex", flexDirection: "column", boxSizing: "border-box" }}>
        ...
        {activeMenu === "paint" ? (
          ...
          <>
          <TR322VideoAudioPage settings={videoSettings} onChange={updVideo} />
          {false && (
